...
state("Yoku") 
    {
        // Values for splits
        string41 LevelName : "Yoku.exe", 0x0054C620, 0x68, 0x130, 0x1A0, 0x88, 0x1F0; //memscan_level\nXXXXXXXX
        string24 GameTime : "Yoku.exe", 0x0054C620, 0x68, 0xD90, 0xF90;  //memscan_time\nXX:XX:XX.xx
        string40 GameItem : "Yoku.exe", 0x0054B2A8, 0x1D8, 0x8, 0x18, 0x518, 0x1B8, 0x778, 0x0; //memscan_lastitem\nxxx:x
    }
...
start {
	if (settings["Speedrun"]) {
		if(current.GameTime.Equals("memscan_time\n00:00:00.01")) { 	
			//Levels
			if (settings["cave_abyssal_access"]){vars.cave_abyssal_access=1; }else{ vars.cave_abyssal_access=0;}
			if (settings["cave_beach_bottom"]){vars.cave_beach_bottom=1; }else{ vars.cave_beach_bottom=0;}
			if (settings["cave_beaver_blockade"]){vars.cave_beaver_blockade=1; }else{ vars.cave_beaver_blockade=0;}
			if (settings["cave_boss"]){vars.cave_boss=1; }else{ vars.cave_boss=0;}
			if (settings["cave_clammy_cenote"]){vars.cave_clammy_cenote=1; }else{ vars.cave_clammy_cenote=0;}
			if (settings["cave_east_bay_shore"]){vars.cave_east_bay_shore=1; }else{ vars.cave_east_bay_shore=0;}
			if (settings["cave_temple_terror_entrance"]){vars.cave_temple_terror_entrance=1; }else{ vars.cave_temple_terror_entrance=0;}
			if (settings["cave_temple_terror_lower"]){vars.cave_temple_terror_lower=1; }else{ vars.cave_temple_terror_lower=0;}
			if (settings["cave_temple_terror_treasury"]){vars.cave_temple_terror_treasury=1; }else{ vars.cave_temple_terror_treasury=0;}
			if (settings["cave_temple_terror_upper"]){vars.cave_temple_terror_upper=1; }else{ vars.cave_temple_terror_upper=0;}
			if (settings["cave_winding_waterway"]){vars.cave_winding_waterway=1; }else{ vars.cave_winding_waterway=0;}			
			if (settings["hub_bowel_bumping_left"]){vars.hub_bowel_bumping_left=1; }else{ vars.hub_bowel_bumping_left=0;}					
			if (settings["hub_cliffside_creek"]){vars.hub_cliffside_creek=1; }else{ vars.hub_cliffside_creek=0;}
			if (settings["hub_festival"]){vars.hub_festival=1; }else{ vars.hub_festival=0;}
			if (settings["hub_festival_sky"]){vars.hub_festival_sky=1; }else{ vars.hub_festival_sky=0;}
			if (settings["hub_festival_upper"]){vars.hub_festival_upper=1; }else{ vars.hub_festival_upper=0;}
			if (settings["hub_hermits_home"]){vars.hub_hermits_home=1; }else{ vars.hub_hermits_home=0;}
			if (settings["hub_island_express"]){vars.hub_island_express=1; }else{ vars.hub_island_express=0;}
			if (settings["hub_left_lemur_lane"]){vars.hub_left_lemur_lane=1; }else{ vars.hub_left_lemur_lane=0;}
			if (settings["hub_left_lofty_logo"]){vars.hub_left_lofty_logo=1; }else{ vars.hub_left_lofty_logo=0;}
			if (settings["hub_marrow_maze"]){vars.hub_marrow_maze=1; }else{ vars.hub_marrow_maze=0;}
			if (settings["hub_obtainium_outland"]){vars.hub_obtainium_outland=1; }else{ vars.hub_obtainium_outland=0;}
			if (settings["hub_soaring_stone"]){vars.hub_soaring_stone=1; }else{ vars.hub_soaring_stone=0;}
			if (settings["hub_village"]){vars.hub_village=1; }else{ vars.hub_village=0;}
			if (settings["hub_white_cliff"]){vars.hub_white_cliff=1; }else{ vars.hub_white_cliff=0;}		
			if (settings["intro_landing"]){vars.intro_landing=1; }else{ vars.intro_landing=0;}
			if (settings["intro_landing_creepy_cavern"]){vars.intro_landing_creepy_cavern=1; }else{ vars.intro_landing_creepy_cavern=0;}
			if (settings["intro_landing_left"]){vars.intro_landing_left=1; }else{ vars.intro_landing_left=0;}
			if (settings["intro_landing_right"]){vars.intro_landing_right=1; }else{ vars.intro_landing_right=0;}
			if (settings["intro_landing_upper"]){vars.intro_landing_upper=1; }else{ vars.intro_landing_upper=0;}
			if (settings["intro_muddled_morass"]){vars.intro_muddled_morass=1; }else{ vars.intro_muddled_morass=0;}
			if (settings["intro_secret"]){vars.intro_secret=1; }else{ vars.intro_secret=0;}								
			if (settings["island_ending_2"]){vars.island_ending_2=1; }else{ vars.island_ending_2=0;}							
			if (settings["jungle_canyon_caper"]){vars.jungle_canyon_caper=1; }else{ vars.jungle_canyon_caper=0;}
			if (settings["jungle_crammed_canopy"]){vars.jungle_crammed_canop=1; }else{ vars.jungle_crammed_canopy=0;}
			if (settings["jungle_misty_meadow"]){vars.jungle_misty_meadow=1; }else{ vars.jungle_misty_meadow=0;}
			if (settings["jungle_mollusc_madness"]){vars.jungle_mollusc_madness=1; }else{ vars.jungle_mollusc_madness=0;}
			if (settings["jungle_roots"]){vars.jungle_roots=1; }else{ vars.jungle_roots=0;}
			if (settings["jungle_secluded_climb"]){vars.jungle_secluded_climb=1; }else{ vars.jungle_secluded_climb=0;}
			if (settings["jungle_slug_struggle"]){vars.jungle_slug_struggle=1; }else{ vars.jungle_slug_struggle=0;}
			if (settings["jungle_spikey_stockade"]){vars.jungle_spikey_stockade=1; }else{ vars.jungle_spikey_stockade=0;}
			if (settings["jungle_tall_tall_tower"]){vars.jungle_tall_tall_tower=1; }else{ vars.jungle_tall_tall_tower=0;}
			if (settings["jungle_willy"]){vars.jungle_willy=1; }else{ vars.jungle_willy=0;}		
			if (settings["peak_aerial_ascent"]){vars.peak_aerial_ascent=1; }else{ vars.peak_aerial_ascent=0;}	
			if (settings["peak_beanstalk_base"]){vars.peak_beanstalk_base=1; }else{ vars.peak_beanstalk_base=0;}	
			if (settings["peak_crooked_cliff"]){vars.peak_crooked_cliff=1; }else{ vars.peak_crooked_cliff=0;}	
			if (settings["peak_crystal_crater"]){vars.peak_crystal_crater=1; }else{ vars.peak_crystal_crater=0;}	
			if (settings["peak_filthy_flat"]){vars.peak_filthy_flat=1; }else{ vars.peak_filthy_flat=0;}	
			if (settings["peak_frostpine_forest"]){vars.peak_frostpine_forest=1; }else{ vars.peak_frostpine_forest=0;}	
			if (settings["peak_guano_grief"]){vars.peak_guano_grief=1; }else{ vars.peak_guano_grief=0;}	
			if (settings["peak_ice_cold_idol"]){vars.peak_ice_cold_idol=1; }else{ vars.peak_ice_cold_idol=0;}	
			if (settings["peak_obtainium_oracle"]){vars.peak_obtainium_oracle=1; }else{ vars.peak_obtainium_oracle=0;}	
			if (settings["peak_sky"]){vars.peak_sky=1; }else{ vars.peak_sky=0;}	
			if (settings["peak_spacemonk_mystery"]){vars.peak_spacemonk_mystery=1; }else{ vars.peak_spacemonk_mystery=0;}	
			if (settings["peak_spider_fight"]){vars.peak_spider_fight=1; }else{ vars.peak_spider_fight=0;}	
			if (settings["peak_underside"]){vars.peak_underside=1; }else{ vars.peak_underside=0;}		
			if (settings["spring_bubbly_basin"]){vars.spring_bubbly_basin=1; }else{ vars.spring_bubbly_basin=0;}	
			if (settings["spring_cloudburst_cliffs"]){vars.spring_cloudburst_cliffs=1; }else{ vars.spring_cloudburst_cliffs=0;}	
			if (settings["spring_gangway_grotto"]){vars.spring_gangway_grotto=1; }else{ vars.spring_gangway_grotto=0;}	
			if (settings["spring_hazy_heaven"]){vars.spring_hazy_heaven=1; }else{ vars.spring_hazy_heaven=0;}	
			if (settings["spring_hidden_hotspring"]){vars.spring_hidden_hotspring=1; }else{ vars.spring_hidden_hotspring=0;}	
			if (settings["spring_jailhouse_japes"]){vars.spring_jailhouse_japes=1; }else{ vars.spring_jailhouse_japes=0;}	
			if (settings["spring_shoddy_shanty"]){vars.spring_shoddy_shanty=1; }else{ vars.spring_shoddy_shanty=0;}	
			if (settings["spring_sleek_slabs"]){vars.spring_sleek_slabs=1; }else{ vars.spring_sleek_slabs=0;}	
			if (settings["spring_tiny_broad"]){vars.spring_tiny_broad=1; }else{ vars.spring_tiny_broad=0;}	
			if (settings["spring_tortoise_tunnel"]){vars.spring_tortoise_tunnel=1; }else{ vars.spring_tortoise_tunnel=0;}	
			if (settings["spring_yokos_yam"]){vars.spring_yokos_yam=1; }else{ vars.spring_yokos_yam=0;}	
			//Items
			if (settings["collectible"]){vars.collectible=1; }else{ vars.collectible=0;}
			if (settings["wallet"]){vars.wallet=1; }else{ vars.wallet=0;}
			if (settings["tadpole"]){vars.tadpole=1; }else{ vars.tadpole=0;}
			if (settings["dustbunny_dirty"]){vars.dustbunny_dirty=1; }else{ vars.dustbunny_dirty=0;}
			if (settings["nugget"]){vars.nugget=1; }else{ vars.nugget=0;}
			if (settings["traitor_spirit"]){vars.traitor_spirit=1; }else{ vars.traitor_spirit=0;}
			if (settings["summons"]){vars.summons=1; }else{ vars.summons=0;}
			if (settings["abilities/map"]){vars.abilities_map=1; }else{ vars.abilities_map=0;}
			if (settings["abilities/partyhorn"]){vars.abilities_partyhorn=1; }else{ vars.abilities_partyhorn=0;}
			if (settings["abilities/mailbag"]){vars.abilities_mailbag=1; }else{ vars.abilities_mailbag=0;}
			if (settings["abilities/speed"]){vars.abilities_speed=1; }else{ vars.abilities_speed=0;}
			if (settings["abilities/double_fruit"]){vars.abilities_double_fruit=1; }else{ vars.abilities_double_fruit=0;}
			if (settings["abilities/slug_vaccum"]){vars.abilities_slug_vaccum=1; }else{ vars.abilities_slug_vaccum=0;}
			if (settings["abilities/slug_upgrade"]){vars.abilities_slug_upgrade=1; }else{ vars.abilities_slug_upgrade=0;}
			if (settings["abilities/kickback"]){vars.abilities_kickback=1; }else{ vars.abilities_kickback=0;}
			if (settings["abilities/dive"]){vars.abilities_dive=1; }else{ vars.abilities_dive=0;}
			if (settings["abilities/dive_speed"]){vars.abilities_dive_speed=1; }else{ vars.abilities_dive_speed=0;}
			if (settings["abilities/hook"]){vars.abilities_hook=1; }else{ vars.abilities_hook=0;}
			if (settings["sootling_leash"]){vars.sootling_leash=1; }else{ vars.sootling_leash=0;}
			if (settings["bucket_empty"]){vars.bucket_empty=1; }else{ vars.bucket_empty=0;}
			if (settings["bucket_full"]){vars.bucket_full=1; }else{ vars.bucket_full=0;}
			if (settings["guano"]){vars.guano=1; }else{ vars.guano=0;}
			if (settings["seed_pod"]){vars.seed_pod=1; }else{ vars.seed_pod=0;}
			if (settings["bluekey"]){vars.bluekey=1; }else{ vars.bluekey=0;}
			if (settings["greenkey"]){vars.greenkey=1; }else{ vars.greenkey=0;}
			if (settings["spring_key"]){vars.spring_key=1; }else{ vars.spring_key=0;}
			if (settings["nim_key"]){vars.nim_key=1; }else{ vars.nim_key=0;}
			if (settings["instrument_pile"]){vars.instrument_pile=1; }else{ vars.instrument_pile=0;}
			if (settings["toolbox"]){vars.toolbox=1; }else{ vars.toolbox=0;}
			if (settings["sad_spider"]){vars.sad_spider=1; }else{ vars.sad_spider=0;}
			if (settings["postal_badge"]){vars.postal_badge=1; }else{ vars.postal_badge=0;}
			if (settings["treasure_map"]){vars.treasure_map=1; }else{ vars.treasure_map=0;}
			if (settings["mushroom_2"]){vars.mushroom_2=1; }else{ vars.mushroom_2=0;}
			if (settings["mushroom_3"]){vars.mushroom_3=1; }else{ vars.mushroom_3=0;}
			if (settings["powerups/skvader_1"]){vars.powerups_skvader_1=1; }else{ vars.powerups_skvader_1=0;}
			if (settings["powerups/skvader_2"]){vars.powerups_skvader_2=1; }else{ vars.powerups_skvader_2=0;}
			if (settings["package_3"]){vars.package_3=1; }else{ vars.package_3=0;}
			if (settings["package_2"]){vars.package_2=1; }else{ vars.package_2=0;}
			if (settings["package_1"]){vars.package_1=1; }else{ vars.package_1=0;}
			if (settings["idol1"]){vars.idol1=1; }else{ vars.idol1=0;}
			if (settings["idol2"]){vars.idol2=1; }else{ vars.idol2=0;}
			if (settings["idol3"]){vars.idol3=1; }else{ vars.idol3=0;}
			if (settings["idol4"]){vars.idol4=1; }else{ vars.idol4=0;}
			if (settings["spores_1"]){vars.spores_1=1; }else{ vars.spores_1=0;}
			if (settings["spores_2"]){vars.spores_2=1; }else{ vars.spores_2=0;}
			if (settings["spores_3"]){vars.spores_3=1; }else{ vars.spores_3=0;}
			if (settings["spores_4"]){vars.spores_4=1; }else{ vars.spores_4=0;}
			if (settings["spores_5"]){vars.spores_5=1; }else{ vars.spores_5=0;}
			if (settings["tracker_caves"]){vars.tracker_caves=1; }else{ vars.tracker_caves=0;}
			if (settings["tracker_jungle"]){vars.tracker_jungle=1; }else{ vars.tracker_jungle=0;}
			if (settings["tracker_peak"]){vars.tracker_peak=1; }else{ vars.tracker_peak=0;}
			if (settings["tracker_springs"]){vars.tracker_springs=1; }else{ vars.tracker_springs=0;}
			if (settings["tracker_scarabs"]){vars.tracker_scarabs=1; }else{ vars.tracker_scarabs=0;}
			if (settings["skins/skin_1"]){vars.skins_skin_1=1; }else{ vars.skins_skin_1=0;}
			if (settings["skins/skin_2"]){vars.skins_skin_2=1; }else{ vars.skins_skin_2=0;}
			if (settings["skins/skin_3"]){vars.skins_skin_3=1; }else{ vars.skins_skin_3=0;}
			if (settings["skins/skin_4"]){vars.skins_skin_4=1; }else{ vars.skins_skin_4=0;}
			if (settings["skins/skin_5"]){vars.skins_skin_5=1; }else{ vars.skins_skin_5=0;}
